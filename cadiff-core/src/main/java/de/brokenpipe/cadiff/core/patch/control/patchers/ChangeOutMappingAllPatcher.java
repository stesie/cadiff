...
public class ChangeOutMappingAllPatcher extends AbstractPatcher implements Patcher {

	private final ChangeOutMappingAllAction action;

	@Override
	public void accept(final PatcherContext context) {
		final var extEl = findExtensionElements(context, action.id());

		final var existingIMA = extEl.getElementsQuery().filterByType(CamundaOut.class).list().stream()
		...
		final var ima = existingIMA.orElseGet(() -> extEl.addExtensionElement(CamundaOut.class));
